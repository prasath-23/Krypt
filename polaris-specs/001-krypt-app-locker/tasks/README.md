...
All WP files are stored flat in `tasks/`. The lane (planned, doing, testing, for_review, done) is stored in the YAML frontmatter `lane:` field.
...
```bash
polaris agent tasks move-task <WPID> --to <lane>
```

Example:
```bash
polaris agent tasks move-task WP01 --to doing
```
