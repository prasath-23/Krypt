...
**Prompt Files**: Each work package references a matching prompt file in `/tasks/` generated by `/polaris.tasks`. Lane status is stored in YAML frontmatter (`lane: planned|doing|testing|for_review|done`). Treat this file as the high-level checklist; keep deep implementation detail inside the prompt files.
